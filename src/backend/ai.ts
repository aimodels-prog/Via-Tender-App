    ...
    experts: {
      ...
      items: {
        ...
        properties: {
          ...
          educationLevel: { type: Type.STRING, description: "CRITICAL: The highest academic degree obtained INCLUDING the specific field of study (e.g., 'PhD in Structural Engineering', 'Master of Science in Architecture', 'Diploma in IT'). NEVER just put 'Diploma'." },
...
export async function runParseCVText(text: string, tax: string[]): Promise<any[]> {
  ...
  14. AGGRESSIVE EDUCATION DETAILS: NEVER extract just "Diploma". You must extract the exact field of study along with the degree (e.g., "Diploma in Civil"). If the CV states the institution country/location, you MUST preserve it in metadata.educations[].location so the generated CV can print it.
